...
int main(int argc, char** argv)
{
  ...
  while(1)
    ...
    if(SSL_library_init()<0)
      {
	fprintf(stderr, "Error opening OpenSSL library\n");
	exit(2);
      }
    ...
    if(SSL_set_fd(ssl, sfd)==0)
      {
	fprintf(stderr, "Error connecting socket to SSL\n");
	exit(2);
      }
    ...
    if(SSL_write(ssl, msg, strlen(msg))<0)
      {
	fprintf(stderr, "Could not verify id\n");
	exit(2);
      }
    fprintf(lg, "ID=%d\n", id);
    fflush(lg);
	...
	while(1)
	{
		...
		if(polls[0].revents & POLLIN)
		{
			int n=0;
			int x=0;
			while(n=SSL_read(ssl, buf+x, 1) > 0 && buf[x] != '\n')
				x++;
			if(n < 0)
			{
